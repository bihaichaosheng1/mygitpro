...
class AutohomeSpider(object):
    ...
    def parse_data(self,spec_datas):
        ...
        for spec_data in spec_datas:
            ...
            a_last_sync_time = datetime.now().strftime("%Y-%m-28 23:35:23")
    ...
    def storing_data(self, car_data):
        ...
        for data in car_data:
            ...
            for s_id, s_name, g_price, c_price, p_price in zip(spec_id, spec_name, guidance_price, cut_price,price):
                data = (brand_id, brand_name, series_id, category_fullname, s_id, s_name, city_id, city_name,g_price,c_price, p_price, contain_brand_id, contain_brand_name, last_sync_time)
                print('抖音表', data)
                # 根据 车型id 城市id 判断 车型是否存在  时间就是确保 库里只保留本月的数据
                sql = "select * from t_car_detail_city_douyin where spec_id='{}'and city_id='{}'and cut_price ='{}' and contain_brand_id ='{}' and last_sync_time='{}'".format(data[4], data[6], data[9], data[11], data[13])
                ...
                autohome_data = (s_id, s_name, city_id, g_price, c_price, p_price, a_last_sync_time)
                print('优惠表', autohome_data)
                # 根据 车型id 城市id 判断 车型是否存在  时间就是确保 库里只保留本月的数据
                autohome_sql = "select * from t_car_detail_city where spec_id='{}'and city_id='{}'and last_sync_time='{}'".format(autohome_data[0], autohome_data[2], autohome_data[6])
                cursor.execute(autohome_sql)
                t_car_detail_city_many = cursor.fetchone()
                print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++',t_car_detail_city_many)
                if t_car_detail_city_many:
                    print('--------------------------------------------------------',t_car_detail_city_many[5], autohome_data[4])
                    if t_car_detail_city_many[5]>str(autohome_data[4]):
                        cursor.execute("insert into t_car_detail_city_cut_price(spec_id,spec_name,city_id,guidance_price,cut_price,price,last_sync_time) value(%s,%s,%s,%s,%s,%s,%s) ",autohome_data)
                        conn.commit()  # 提交数据
                        print('降价数据已提交')

                        # 降价表更新操作
                        updata_sql = "UPDATE t_car_detail_city SET cut_price='{}',price='{}' where spec_id = '{}'and city_id = '{}'".format(autohome_data[4], autohome_data[5], autohome_data[0], autohome_data[2])
                        cursor.execute(updata_sql)
                        conn.commit()
                        print('降价数据已更新')

                    # 凯美瑞 2.5G 豪华版 国VI2019款 10000
                    print('优惠表中已存在')
                    updata_sql = "UPDATE t_car_detail_city SET cut_price='{}',price='{}' where spec_id = '{}'and city_id = '{}'".format(autohome_data[4],autohome_data[5],autohome_data[0],autohome_data[2])
                    cursor.execute(updata_sql)
                    conn.commit()
                    ...
                    cursor.execute("insert into t_car_detail_city(spec_id,spec_name,city_id,guidance_price,cut_price,price,last_sync_time) value(%s,%s,%s,%s,%s,%s,%s) ",autohome_data)
                    conn.commit()  # 提交数据
                    print('优惠数据已提交')

                """汽车之家城市降价表插入数据"""
                # t_car_detail_city_cut_price_sql ="select * from t_car_detail_city where spec_id='{}'and city_id='{}'and last_sync_time='{}'".format(autohome_data[0], autohome_data[2], autohome_data[6])
                # # print(t_car_detail_city_cut_price_sql)
                # cursor.execute(t_car_detail_city_cut_price_sql)
                # t_car_detail_city_cut_price_many = cursor.fetchone()
                # if t_car_detail_city_cut_price_many:
                #     print('降价表中已存在')
                #     print(t_car_detail_city_cut_price_many[5],autohome_data[4])

                # else:
                #     cursor.execute("insert into t_car_detail_city_cut_price(spec_id,spec_name,city_id,guidance_price,cut_price,price,last_sync_time) value(%s,%s,%s,%s,%s,%s,%s) ",
                #         autohome_data)
                #     conn.commit()  # 提交数据
                #     print('降价数据已提交')
    ...
    def query_data(self):
        city_id = [210100,210200,210300,210400,210500,210600,210700,210800,210900,211000,211100,211200,211300,211400]
        return city_id
...
#     """数据库类"""
#
#     def __init__(self, host='localhost', user='root', pw='',port=3306, db='jdbc'):
#         self.con = pymysql.connect(host, user, pw,port, db)
